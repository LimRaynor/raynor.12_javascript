...
console.log("[배열의 생성자와 프로토타입(부모)]");
// Array == 생성자 함수
// arr.constructor === arr 배열을 만들때 사용한 생성자 함수
console.log(arr.constructor === Array) // true

// 배열의 프로토타입(부모) 객체
console.log(Object.getPrototypeOf(arr) === Array.prototype)// 둘의 부모가 같은가

// Array.prototype.indexOf -> arr의
//  indexOf(요소) : 배열 "시작" 부터 검색해서 처음 찾은 요소의 인덱스를 반환 (없으면 -1)
//  LastIndexOf(요소) : 배열 "반대방향(마지막)" 부터 검색해서 처음 찾은 요소의 인덱스를 반환 (없으면 -1)
// includes(요소) : 배열에 해당 요소가 있으면 true, 없으면 false

const foodList = ['물회', '삼계탕', '냉면', '수박', '물회'];
console.log('원본 배열:', foodList);

console.log("arr.indexOf('물회') : ", foodList.indexOf('물회')); // 처음 부터
console.log("arr.indexOf('물회', 2) : ", foodList.indexOf('물회', 1));
console.log("arr.indexOf('햄버거', 2) : ", foodList.indexOf('햄버거', 1));

console.log("arr.lastIndexOf('물회') : ", foodList.lastIndexOf('물회')); // 뒤 부터
console.log("arr.lastIndexOf('물회', 1) : ", foodList.lastIndexOf('물회', 1));
console.log("arr.lastIndexOf('햄버거', 1) : ", foodList.lastIndexOf('햄버거', 1));

console.log("foodList.incloudes('물회'): ", foodList.includes('물회'));
console.log("foodList.incloudes('삼겹살'): ", foodList.includes('삼겹살'));

console.log('='.repeat(50));

// Array.prototype.push() // 마지막 요소로 추가
// Array.prototype.pop() // 마지막 요소를 꺼넨후 삭제
// Array.prototype.unshift() // 첫번째 요소로 추가
// Array.prototype.shift // 첫 번째 요소를 꺼네서 제거

const chineseFood= ['짜장면', '짬뽕', '유산슬'];
console.log("초기 배열 chineseFood : ", chineseFood);

// push
chineseFood.push('탕수육');
chineseFood.push('찹쌀 탕수육');
...
// Array.prototype.concat() // 두 개 이상의 배열을 결함
const idol1 = ['아이브', 'BTS'];
