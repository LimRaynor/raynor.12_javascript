...
// student의 프로토타입을 user로 지정
//  -> student가 user을 상속받았다고 표현
//      -> student가 user로 부터 상속 받은 프로퍼티
//      == 상속 프로퍼티(라고 부름)
student.__proto__ = user; // user을 부모로 지정

console.log(`student.passion: ${student.passion}`);
console.log(`student.activate: ${student.activate}`); // activate는 상속 프로퍼티이다
student.login();
