...
// 숫자가 아닌 문자열은 제외하고 변환 -> 숫자면 제외 가능
console.log(parseFloat('10cm'));        // 10

// 첫 번째 문자열을 숫자로 변환할 수 없다면 변환 불가 -> 숫자가아니면 제외 불가
...
// 이스케이프 처리를 위해 인코딩(압축, 암호화)한다.
// 네트워크를 통해 정보를 공유할 때 어떤 시스템에서도 읽을 수 있는 아스키 문자 셋으로 변환
const uri = 'http://greedy.com?name=홍길동&job=student';
const enc = global.encodeURI(uri);
console.log(enc);

// decodeURI
// decodeURI('http://greedy.com?name=%ED%99%8D%EA%B8%B8%EB%8F%99&job=student')
// 인코딩 된 URI를 인수로 전달 받아 이스케이프 처리 이전으로 디코딩한다.
const dec = decodeURI(enc);
console.log(dec);

// encodeURIComponent
// URI 구성 요소를 인수로 전달 받아
// 알파벳, 0~9의 숫자, - _ . ! ~ * ' ( ) 문자는 제외하고 인코딩한다.
