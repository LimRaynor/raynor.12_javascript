/* 01_spread-syntax */
// 스프레드 문법(전개 문법)
// - 반복 가능한(iterable) 객체(배열, 문자열, 객체)
// 각각 개별 요소롤 분리하여 목록을 만드는 문법 == 배열에서 [] 을 벗기는 문법

const arr = [10, 20, 30]

console.log(`가장 큰 값 : ${Math.max(arr)}`); // 배열 X
console.log(`가장 큰 값 : ${Math.max(arr[0], arr[1], arr[2])}`); // 하나씩 넣어줘야 함
console.log(`가장 큰 값 : ${Math.max(...arr)}`); // 어떼요 참 쉽죠? 위에껄 줄임

const arr1  = [10, 20, 30];
const arr2  = [110, 220, 330];
...
console.log(...str); // J a v a S c i p t
console.log("----------[] 장착-----------")
console.log([...str]); // 배열로 만들어줌
