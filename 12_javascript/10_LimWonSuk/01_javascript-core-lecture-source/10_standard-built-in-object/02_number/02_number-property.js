...
console.log(Number.MAX_VALUE); // e의 308승
...
// Number.EPSILON : "부동 소수점"으로 인해 발생하는 오차를 해결하기 위해 사용한다
console.log(0.1 + 0.2); // 0.30000000000000004
// 2진수로 소수 표현 시 무한 소수가 발생되어 오차가 발생함 (컴퓨터는 유한하여서)

console.log(0.1 + 0.2 === 0.3); // falselog
console.log(Number.EPSILON); // 오차 해결을 위한 숫자,
// 1과 1보다 큰 숫자 중에서 가장 작은 숫자와의 차이를 나타내는 숫자

function isEqual(a, b){
    // a - b 의 절대 값이 Number.EPSILON 보다 작으면 같은 수로 인정함
    return Math.abs(a - b) < Number.EPSILON;
}
console.log(isEqual(0.1 + 0.2, 0.3)); // 실수가 확실하지 않을때만 사용
