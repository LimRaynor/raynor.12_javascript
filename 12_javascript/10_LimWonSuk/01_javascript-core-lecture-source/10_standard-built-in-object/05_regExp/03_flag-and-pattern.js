...
console.log(target.match(/VA/i)); // /i == 대소문자 상관없이 ->  index == 2
console.log(target.match(/VA/ig)); // /i + /g -> ["va","va"]
...
console.log(target.match(/abcd.../)) // . 하나당 한글자

// {m, n} :최소 m번, 최대 n번 반복되는 문자열(반복 검색)
target = 'a aa aaa b bb bbb ab aab abb bbbbb';
console.log(target.match(/a{2,3}/g));  // a 최소 2번~ 최대 3번 반복
console.log(target.match(/b{2}/g)); // b두번 반복 -> bb
console.log(target.match(/b{3,}/g)); // b 3번 이상 반복 -> bbb

console.log('-----------id검사시 사용하는 것들-----------');

// + : 앞선 패턴이 최소 한 번 이상 반복되는 문자열(반복 검색)
// +는 {1,}과 같다.
console.log(target.match(/b+/g)); // b 가 하나 이상존재하는거 /전역

// * : 앞선 패턴이 최소 0번 이상 반복되는 문자열(반복 검색)
// *는 {0,}
console.log(target.match(/b*/g)); // b가 있을수도 있고 없을수도 있다 -> b다 찾아라


// ? : 앞선 패턴이 최대 한 번(0번 포함) 포함되는 문자열
// ?는 {0,1}과 같다.
target = 'soul seoul seul';
console.log(target.match(/se?oul/g)); // se가 0개거나 1개이상 인경우 /전역
...
// [] 내의 문자는 or로 동작
console.log(target.match(/[abc]+/g)); // {}안에서 하나라도 있는가
// 범위를 지정하려면 - 사용
// [a-z] -> 한 글자가 영어 소문자 범위의 글자인 경우
console.log(target.match(/[a-z]+/g));
// [A-Z] -> 한 글자가 영어 대문자 범위의 글자인 경우
console.log(target.match(/[A-Z]+/g));
// 대소문자 범위
console.log(target.match(/[A-Za-z]+/g));
console.log(target.match(/[A-Za-z0-9]+/g)); // 10진수 == digit ==
...
// [] 내의 ^ : not(부정)
console.log(target.match(/[^0-9]+/g)); // 숫자 아닌거 찾기
console.log(target.match(/[^a-z]+/g)); //  소문자 아닌거 찾기
console.log('----------------------------');
